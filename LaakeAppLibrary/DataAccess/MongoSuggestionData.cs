...
//muista kun on valmis lisätä "extract interface", jotta tämä ": ISuggestionData" ilmestyy 
public class MongoSuggestionData : ISuggestionData
{
   ...
   public async Task<List<SuggestionModel>> GetAllSuggestions()
   {
      ...
      if (output is null)
      {
         ...
         //vain minuutin cachissa, niin haku ei "ylikuumene"
         _cache.Set(CacheName, output, TimeSpan.FromMinutes(1));
      }
      return output;
   }
   
   //palauttaa käyttäjän kaikki ehdotukset
   ...
   public async Task<List<SuggestionModel>> GetAllApprovedSuggestions()
   {
      ...
      //mutta määritellään eri where lisämääreellä
   ...
   public async Task<List<SuggestionModel>> GetAllSuggestionsWaitingForApproval()
   {
      ...
      x.ApprovedForRelease == false && x.Rejected == false).ToList();
   }
   ...
   public async Task UpvoteSuggestion(string suggestionId, string userId)
   {
      ...
      //kun kirjoitetaan kahteen niin halutaan varmistaa, että onnistui molempiin
      ...
      try
      {
         ...
         //First ei FirstOrDefault koska muuten heittäisi nulliksi eikä voi äänestää mitä ei ole
         var suggestion = (await suggestionsInTransaction.FindAsync(s => s.Id == suggestionId)).First();

         //koska se on hashset niin se hyväksyy vain uniikit, eli jos se onnistuu niin palauttaa true
         //myös mahdollista yhdestä napista lisätä ja poistaa omat äänet
